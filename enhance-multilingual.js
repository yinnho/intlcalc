...
function generateLanguageSwitcher(currentLang) {
    ...
    return '\n    <div class="language-switcher">\n        <div class="current-lang">' + 
           ...
           translations['other-languages'][currentLang] + 
           '</span>\n                <div class="lang-menu">\n                    ' + 
           langLinks + 
           '\n                </div>\n            </div>\n        </div>\n    </div>';
}
...
// 生成面包屑导航HTML
function generateBreadcrumb(currentLang, page, calculatorId = null) {
    const homeText = translations.home[currentLang];
    let breadcrumb = '<nav class="breadcrumb">\n        <a href="/">' + homeText + '</a>';
    
    if (page === 'calculator' && calculatorId) {
        const category = calculatorCategories.calculators.includes(calculatorId) ? 'calculators' : 'converters';
        const categoryText = translations[category][currentLang];
        const calculatorText = calculatorTranslations[calculatorId][currentLang].title;
        
        breadcrumb += ' <span class="separator">></span> <span class="category">' + categoryText + '</span>';
        breadcrumb += ' <span class="separator">></span> <span class="current">' + calculatorText + '</span>';
    }
...
// 生成相关计算器HTML
function generateRelatedCalculators(currentLang, calculatorId) {
    const related = relatedCalculators[calculatorId] || [];
    if (related.length === 0) return '';
    
    const relatedHtml = related.map(id => {
        const calc = calculatorTranslations[id][currentLang];
        const category = calculatorCategories.calculators.includes(id) ? 'calc' : 'convert';
        return '\n            <div class="related-item">\n                <a href="/' + 
               category + '/' + id + '.html">' + calc.title + 
               '</a>\n                <p>' + calc.desc + '</p>\n            </div>';
    }).join('');
    
    return '\n    <div class="related-calculators">\n        <h3>' + 
           translations['related-tools'][currentLang] + 
...
// 生成其他语言版本链接HTML
function generateOtherLanguageVersions(currentLang, calculatorId) {
    const category = calculatorCategories.calculators.includes(calculatorId) ? 'calc' : 'convert';
...
function enhanceIndexPage(langDir, lang) {
    ...
    // 更新页面标题和描述的翻译
    content = content.replace(/(<h1>)[^<]+(</h1>)/, '$1International ' + translations.calculators[lang] + '$2');
    content = content.replace(/(<h2>📊\s)[^<]+(</h2>)/, '$1' + translations.calculators[lang] + '$2');
    content = content.replace(/(<h2>🔄\s)[^<]+(</h2>)/, '$1' + translations.converters[lang] + '$2');
    ...
    Object.entries(calculatorTranslations).forEach(([id, calcTranslations]) => {
        if (calcTranslations[lang]) {
            ...
            content = content.replace(regex, '$1' + calcTranslations[lang].title + '$2');
            
            const descRegex = new RegExp('(<a href="[^"]*' + id + '\\.html">[^<]+</a></h3>\\s*<p>)[^<]+(</p>)', 'g');
            content = content.replace(descRegex, '$1' + calcTranslations[lang].desc + '$2');
        }
    });
    
    // 添加语言切换器
    const langSwitcher = generateLanguageSwitcher(lang);
    content = content.replace(/(<body>)/, '$1' + langSwitcher);
    
    // 添加样式
    const styleRegex = /(</style>)/;
    content = content.replace(styleRegex, enhancedStyles + '$1');
...
function enhanceCalculatorPage(langDir, lang, calculatorId, category) {
    ...
    // 更新页面标题
    content = content.replace(/(<title>)[^<]+(</title>)/, '$1' + calcTrans.title + '$2');
    content = content.replace(/(<h1[^>]*>)[^<]+(</h1>)/, '$1' + calcTrans.title + '$2');
    
    // 更新meta描述
    content = content.replace(/(<meta name="description" content=")[^"]+(")/, '$1' + calcTrans.desc + '$2');
    ...
    // 添加面包屑导航
    const breadcrumb = generateBreadcrumb(lang, 'calculator', calculatorId);
    content = content.replace(/(<div class="[^"]*container[^"]*">)/, '$1' + breadcrumb);
    
    // 添加相关计算器
    const relatedCalcs = generateRelatedCalculators(lang, calculatorId);
    
    // 添加其他语言版本
    const otherLangVersions = generateOtherLanguageVersions(lang, calculatorId);
